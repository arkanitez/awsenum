...
def enumerate_ec2(session, region: str, g: Graph, warnings: List[str]):
    ...
    # FIX: Using the refactored helper for both ingress and egress rules.
    for sg in sgs.get('SecurityGroups', []):
        sgid = sg['GroupId']
        _process_sg_permissions(sg.get('IpPermissions', []), sgid, "ingress", g, region)
        _process_sg_permissions(sg.get('IpPermissionsEgress', []), sgid, "egress", g, region)
...
def enumerate_elbv2(session, region: str, g: Graph, warnings: List[str]):
    ...
    # FIX: Removed redundant ec2 client creation.
    # ec2 = session.client('ec2', region_name=region, config=BOTO_CFG)
...
# FIX: Removed the overly permissive CORS middleware. For a local tool serving
# its own frontend, this is unnecessary and a security risk if misconfigured.
# app.add_middleware(
#     CORSMiddleware,
#     allow_origins=["*"],
...
<head>
  ...
  <style>
    ...
    input[type=text], input[type=password] { width:100%; box-sizing: border-box; padding:8px; border:1px solid #ddd; border-radius:8px; font-size:13px; }
...
if __name__ == "__main__":
    ...
    import uvicorn
    # It's recommended to run on 127.0.0.1 to avoid exposing the
    # credential-handling endpoint to the network.
    uvicorn.run("awsenum:app", host="127.0.0.1", port=8000, reload=False)
